<template>
  <div id="filterSystem" style="display: flex; flex-wrap: wrap">
    <div v-for="(column, columnIndex) in myQuery.predicates" :key="columnIndex">
      <div v-if="columnIndex > 0" class="dropdown">
        ...
        >
          <small class="p-0"
            ><strong>{{ myQuery.type }}</strong></small
          >
        </button>
      </div>

      <div class="my-2" v-if="column.predicates.length > 0">
        <div
          v-for="(columnInGroup, columnIndex) in column.predicates"
          :key="columnIndex"
        >
          <div v-if="columnIndex > 0" class="dropdown">
            <ul class="dropdown-menu dropdown-menu-right">
            ...
            >
              <span class=""
                ><strong>{{ column.type }}</strong></span
              >
            </button>
          </div>
          ...
          <div>
              ...
              <form v-on:submit.prevent class="px-4 py-3">
                ...
                >
                  ...
                  >
                    ...
                    >
                      <option
                        v-for="item in columnInGroup.list"
                        :key="item.id"
                        :value="item.id"
                      >
                        {{ item.name }}
            ...
            <button
              :class="
                isEmptyColumn(columnInGroup)
                  ? 'btn p-0 button_custom_empty'
                  : 'btn p-0 '
              "
              @mouseover="columnInGroup.onHover = true"
              @mouseleave="columnInGroup.onHover = false"
              type="button"
              :id="columnInGroup.id + '-button-group'"
              data-toggle="dropdown"
              aria-haspopup="true"
              aria-expanded="true"
            >
              <div style="position: relative">
                <small class="pl-2 pt-2 pb-2 mr-1"
                  ><strong>{{ columnInGroup.name }} </strong></small
                >
                <small class="pt-2 pb-2 pr-2" v-if="columnInGroup.value"
                  >{{ columnInGroup.label }}
                  <i>"{{ getChooseValue(columnInGroup) }}"</i>
                </small>
                <small class="pt-2 pb-2 pr-2" v-else>
                  <small
                    v-if="
                      columnInGroup.comparison == 'unknown' ||
                      columnInGroup.comparison == 'known'
                    "
                  >
                    {{ columnInGroup.comparison }}
                  </small>
                  <span v-else>is missing value </span>
                </small>

                <i
                  class="rounded-circle icon-close btn-danger btn btn-sm"
                  style="
                    right: 0;
                    top: -5px;
                    position: absolute;
          ...
          <div v-if="columnInGroup.canAddFilter">
            ...
            <div class="dropdown-menu dropdown-menu-right scrollable-menu">
              ...
              <ul>
                <li
                  v-for="(option, index) in getColumnsFiltered()"
                  class="dropdown-item"
                  :key="index"
          ...
          <div v-if="columnInGroup.canCreateGroup">
            <button
              class="btn btn-secondary p-0 px-1"
              type="button"
              data-toggle="dropdown"
              aria-haspopup="true"
              aria-expanded="true"
            ...
            >
              <span class="p-0"><strong>+</strong></span>
            </button>
    ...
    <div class="dropdown w-100">
      <button
        class="w-100 btn btn-primary themed-button btn-sm p-0"
        type="button"
        data-toggle="dropdown"
        aria-haspopup="true"
        aria-expanded="true"
      >
        <span class="p-1" style="font-size: 18px">{{
...
export default {
  ...
  created() {
    if (this.filterSystem.predicates) {
      this.myQuery = this.filterSystem;
      this.oldQuery = this.filterOldQuery;
  ...
  methods: {
    getChooseValue(columnInGroup) {
      if (columnInGroup.list !== undefined) {
        let r = columnInGroup.list.filter((c) => c.id == columnInGroup.value);
        if (r.length > 0) {
          if (r[0].label != undefined) {
            return r[0].label;
          } else if (r[0].name != undefined) {
            return r[0].name;
          }
        }
      }
      return columnInGroup.value;
    },
    checkCallApi() {
      let newQuery = "";
      newQuery += this.myQuery.sort + " ";
      newQuery += this.myQuery.dir + " ";
      let i = 0;
      for (let column of this.myQuery.predicates) {
        i += 1;
        if (column.type == "and" || column.type == "or") {
          if (i > 1) {
            newQuery += this.myQuery.type + " ";
          }
        }
        let j = 0;
        for (let innerColumn of column.predicates) {
          if (
            innerColumn.comparison != "unknown" &&
          ...
          if (innerColumn.value != "") {
            newQuery +=
              "(" + innerColumn.comparison + ")" + innerColumn.value + " ";
          }
          if (
            innerColumn.comparison == "unknown" ||
            innerColumn.comparison == "known" ||
      ...
      if (
        ...
        this.$emit("callApi", {
          filters: this.myQuery,
          oldQuery: this.oldQuery,
        });
      }
    },
    radioClick(column) {
    ...
    removeButton(event, column) {
      ...
      this.myQuery.predicates = this.myQuery.predicates.filter(
        (c) => c.id != column.id
      );
      for (let myColumn of this.myQuery.predicates) {
        if (
          myColumn.predicates != undefined &&
          myColumn.predicates.length > 0
        ) {
          myColumn.predicates = myColumn.predicates.filter(
            (c) => c.id != column.id
          );
          if (myColumn.predicates.length > 0) {
            if (myColumn.predicates.length > 1) {
              myColumn.predicates[
                myColumn.predicates.length - 1
              ].canAddFilter = true;
            }
          } else {
            //remove empty group
    ...
    createGroup(columnToAdd, columnBefore) {
      ...
      columnBefore.predicates[
        columnBefore.predicates.length - 1
      ].canCreateGroup = false;
      columnBefore.predicates.push(newColumn);

      setTimeout(() => {
        $("#" + newColumn.id + "-button-group").trigger("click");
      }, 100);
    },
    ...
    addFilter(columnValues, columnGroup) {
      ...
      if (columnGroup != undefined) {
        columnGroup.predicates[
          columnGroup.predicates.length - 1
        ].canAddFilter = false;
...
#filterSystem .dropdown-menu.dropdown-menu-right {
  width: 267px;
}

#filterSystem .btn.btn-primary.themed-button.btn-sm span {
