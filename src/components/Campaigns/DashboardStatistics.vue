<template>
  <div>
    ...
    <div class=" animated fadeIn faster"  v-if="!loadingStats">  
          ...
          <div class="content-chart" >
            <div class="card no-border">
              ...
              <div  v-if="!loadingStats">
                <line-chart :width="300" :height="100"  :chart-data="multiaxis" :options="multiaxis.options" />
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="contenedor animated fadeIn faster d-none">
      <div class="eff-blur" v-if="(currentUser.role_id === '2' && checkBilling === true)" ></div>
        ...
        <div class="col-md-4">
          <div class="content-chart" >
            <div class="card no-border">
              <LoadingSpinner v-if="loadingStats" />
              <div  v-if="!loadingStats">
                <h5 class="title py-2 text-center" >{{ $t('lang.label.industry') }}</h5>
                <radar-chart class="py-2"  :chartdata="industry" :options="industry" />
              </div>
            </div>
          </div>
        </div>        
        <div class="col-md-4">
          <div class="content-chart" >
            <div class="card no-border">
              <LoadingSpinner v-if="loadingStats" />
              <div  v-if="!loadingStats">
                <h5 class="title py-2 text-center" >{{ $t('lang.label.hierarchicallevel') }}</h5>
                <radar-chart class="py-2"  :chartdata="hierarchicallevel" :options="hierarchicallevel.options" />
              </div>
            </div>
          </div>
        </div>
        <div class="col-md-4">
          <div class="content-chart" >
            <div class="card no-border">
              <LoadingSpinner v-if="loadingStats" />
              <div  v-if="!loadingStats">
                <h5 class="title py-2 text-center" >{{ $t('lang.label.area') }}</h5>
                <radar-chart class="py-2"  :chartdata="area" :options="area.options" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="row mt-2">
        <div class="col-md-4 d-none">
          <div class="card py-2 px-3 no-border mb-auto">
            <h5 class="title py-2 text-center" >{{ $t('lang.label.topurl') }}</h5>
            <div class="table">
        ...
        <div class="col-md-4 d-none">
          <div class="content-chart" >
            <div class="card no-border">
              <LoadingSpinner v-if="loadingStats" />
              <div  v-if="!loadingStats">
                <h5 class="title py-2 text-center" >{{ $t('lang.label.sessionquantity') }}</h5>
                <doughnut-chart class="py-2"  :chartdata="sessionquantity" :options="sessionquantity.options" />
              </div>
            </div>
          </div>
        </div>
        <div class="col-md-4 d-none">
          <div class="content-chart" >
            <div class="card no-border">
              <LoadingSpinner v-if="loadingStats" />
              <div  v-if="!loadingStats">
                <h5 class="title py-2 text-center" >{{ $t('lang.label.sessionsduration') }}</h5>
                <doughnut-chart class="py-2"  :chartdata="timesessions" :options="timesessions.options" />
              </div>
            </div>
          </div>
        </div>
    ...
    return {
      ...
      multiaxis: {
        ...
        datasets: [
          /*{
          type: 'bar',
          label: this.$t('lang.label.expenses'),
          hidden: true,
          borderColor: 'RGBA(81,85,234,1)',
          backgroundColor: 'RGBA(81,85,234,0.8)',
      ...
      area: {
        labels: ['Ile de France', 'Paca', 'Bretagne', 'Franche-Compté', 'Bourgogne', 'Nord'],
        datasets: [{
          borderColor: ['#ff6384'],
          label: this.$t('lang.label.answers'),
          data: [3,10, 4,3,10, 4]
      ...
      sessionquantity: {
        datasets: [{
          ...
          backgroundColor: ["#ffce56","#36a2eb","#185190","#ff6384", "#ff9f40"],
          hoverBackgroundColor: ["#fbd2cd","#fef5c9","#d1e3f7","#fbd2cd", "#fef5c9"]
        }],
        labels: ["1","2","3","4","5+"],
        option: {},
      },
  ...
  alertShowCampaign(){
    ...
    loadStatsData(){
      if (!this.loadingStats){
        ...
        //this.multiaxis.datasets[0].data = this.stats.activity_by_date.activity_price_total;
        this.multiaxis.datasets[0].data = this.stats.activity_by_date.emails_sent;
        this.multiaxis.datasets[1].data = this.stats.activity_by_date.emails_opened;
        this.multiaxis.datasets[2].data = this.stats.activity_by_date.activity_total;
        this.multiaxis.datasets[3].data = this.stats.activity_by_date.email_response_total;
        this.multiaxis.datasets[4].data = this.stats.activity_by_date.bounce_total;
      }
    },
    validarDate(){ 
